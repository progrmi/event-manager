...
exports.updateProfile = async (req, res) => {
  const { full_name, password, confirmPassword } = req.body;
  ...
  try {
    const updateData = { userId, full_name };
    ...
    // Generate a new token with the updated full name
    const token = jwt.sign({ userId, email, full_name }, JWT_SECRET, {
      expiresIn: "24h",
    });
    ...
    res.locals.user.full_name = full_name;
    res.render("profile", {
      title: "Your Profile",
